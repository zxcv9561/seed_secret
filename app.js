...
async function loadStateFromSupabase() {
  ...
  try {
    ...
    var g = extracted[0], a = extracted[1], ab = extracted[2], cs = extracted[3],
        ...
        rx = extracted[16], crx = extracted[17], ft = extracted[18];
...
function renderAboutDetail(view, id) {
  ...
  if (canModify) {
    bindEditToggleButtons(hdr, function() {
      if (titleEl && editable) {
        var newTitle = (titleEl.innerText || '').trim();
...
function renderAgentDetail(view, id) {
  ...
  if (canModify) {
    bindEditToggleButtons(hdr, function() {
      if (titleEl && editable) {
        var newName = (titleEl.innerText || '').trim();
        if (newName !== a.name) {
          a.name = newName;
          saveEntity('agent', a.id);
        }
