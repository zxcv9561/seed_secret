...
function groupToRow(g, order) {
  return { id: g.id, name: g.name, sort_order: order || 0 };
}
...
async function loadStateFromSupabase() {
  ...
  try {
    ...
    var g = extracted[0], a = extracted[1], ab = extracted[2], cs = extracted[3],
        ...
        rx = extracted[16], crx = extracted[17];
    ...
    var groups = (g.data || []).map(function(row) { return { id: row.id, name: row.name, agents: [] }; });
    var groupMap = {};
...
function renderBlock(block, idx, blocks, onChange, saveOwner, readOnly) {
  ...
  if (!readOnly) {
    ...
    handle.querySelectorAll('.block-btn').forEach(function(b) {
...
function renderAboutList(view) {
  view.appendChild(sectionHeader('기관 소개', 'About the Bureau',
    ...
    function() {
      showPrompt('새 항목 추가', '서브 항목의 제목을 입력하세요').then(function(v) {
        if (!v) return;
        var item = {
        ...
        item.index = romanize(state.about.length + 1);
  ...
  visible.forEach(function(item, i) {
    ...
    var fav = isFavorited('about', item.id) ? ' <span style="color:var(--class-yellow)">★</span>' : '';
    card.innerHTML =
      '<div>' +
        '<div class="ac-index">§ ' + esc(item.index) + ' ' + visibilityBadge(item, 'about') + '</div>' +
        '<div class="ac-title">' + esc(item.title) + fav + '</div>' +
      '</div>' +
      '<div class="ac-meta">블록 ' + item.blocks.length + '개</div>';
    card.onclick = function() { openDetail('about', item.id); };
...
function renderAboutDetail(view, id) {
  ...
  var editable = canEdit(item, "about");
...
function renderCaseDetail(view, id) {
  ...
  var page = document.createElement('div');
  page.className = 'detail-page';
  var editable = canEdit(c, state.detail && state.detail.type);
...
function renderDossierDetail(view, id) {
  ...
  var page = document.createElement('div');
  page.className = 'detail-page';
  var editable = canEdit(d, 'dossier');
...
function renderAgentsList(view) {
  view.appendChild(sectionHeader('요원 명부', 'Personnel Roster',
    isMaster() ? '+ 소속 생성' : null,
    function() {
      showPrompt('새 소속 생성', '소속(부대·부서) 이름을 입력하세요', 'WIZARD-Ⅶ').then(function(v) {
        if (!v) return;
        var g = { id: genId('group'), name: v, agents: [] };
  ...
  state.agentGroups.forEach(function(g) {
    // 요원 명부 리스트는 섹션 view 권한만 있으면 모든 요원 표시 (개별 visibility는 상세 페이지에서만 체크)
    // Master는 항상 모두 보임
    ...
    // Hide group entirely if searching and no matches
    if (q && visibleAgents.length === 0) return;
    ...
    if (delBtn2) delBtn2.onclick = function() {
      showConfirm('소속 삭제', '「' + g.name + '」 소속을 삭제합니다. 소속된 요원(' + g.agents.length + '명)과 그들의 사진·이미지·이모티콘·첨부파일이 모두 저장소에서 제거됩니다.', '삭제').then(function(v) {
        if (!v) return;
        // Collect all storage URLs from members
        var urls = [];
        ...
        // Remove emoticon rows from state and DB
        var orphanEmo = state.emoticons.filter(function(e) {
          return g.agents.some(function(a) { return a.id === e.ownerId; });
        });
        orphanEmo.forEach(function(e) { deleteEntity('emoticons', e.id); });
        state.emoticons = state.emoticons.filter(function(e) {
          return !g.agents.some(function(a) { return a.id === e.ownerId; });
        });

        state.agentGroups = state.agentGroups.filter(function(x) { return x.id !== g.id; });
        // DB cascade will delete agent rows (ON DELETE CASCADE on agents.group_id)
        deleteEntity('agent_groups', g.id);
        render();
      });
    };
    ...
    if (visibleAgents.length === 0) {
      ...
      empty.style.cssText = 'padding:24px; text-align:center; border:1px dashed var(--rule); background:var(--paper-2); font-family:var(--font-mono); font-size:11px; letter-spacing:0.16em; color:var(--ink-faint); text-transform:uppercase;';
      empty.textContent = '● 등록된 요원 없음';
      ...
      visibleAgents.forEach(function(a) {
        ...
        card.innerHTML =
          ...
          '<div>' +
            ...
            accountHint +
          '</div>';
        card.onclick = function() { openDetail('agent', a.id); };
        grid.appendChild(card);
      });
...
function renderAgentDetail(view, id) {
  ...
  var page = document.createElement('div');
  page.className = 'detail-page';
  // For agents: editable if master, or this is my own profile, or I'm in editor_ids
  var editable = isMaster() || (currentUser && currentUser.agentId === a.id) || canEdit(a, 'agent');
  ...
  hdr.innerHTML =
    ...
    '<div class="detail-header-row">' +
      ...
      '<div class="detail-actions">' +
        ...
        permButton(a, 'agent') +
        (editable && isMaster() ? '<button class="btn-danger" id="del-btn">● 삭제</button>' : '') +
      '</div>' +
    '</div>';
  ...
  if (editable) {
    ...
    if (delBtn) delBtn.onclick = function() {
      showConfirm('요원 삭제', '「' + a.name + '」 요원을 명부에서 삭제합니다.\n(프로필 사진, 블록 이미지, 이모티콘 모두 저장소에서 제거됩니다)', '삭제').then(function(v) {
        if (!v) return;
        // Collect all storage URLs owned by this agent
        var urls = [];
        if (a.photo) urls.push(a.photo);
        urls = urls.concat(collectBlockUrls(a.blocks));
        // Emoticons owned by this agent
        state.emoticons.forEach(function(e) {
          if (e.ownerId === a.id && e.url) urls.push(e.url);
        });
        deleteStorageFiles(urls);
        // Remove emoticon rows from DB
        state.emoticons.filter(function(e) { return e.ownerId === a.id; }).forEach(function(e) {
...
function renderBoardItems(view) {
  ...
  if (state.boardView === 'grid') {
    ...
    visible.forEach(function(p) {
      ...
      card.innerHTML =
        ...
        '<div class="bc-content">' +
          '<div class="bc-title">' + esc(p.title) + (isFavorited('post', p.id) ? ' <span style="color:var(--class-yellow)">★</span>' : '') + '</div>' +
          '<div class="bc-meta">' +
    ...
    visible.forEach(function(p) {
      ...
      countCell += '</td>';
      tr.innerHTML =
        '<td class="col-title">' + esc(p.title) + (isFavorited('post', p.id) ? ' <span style="color:var(--class-yellow)">★</span>' : '') + (p.visibility === 'private' ? ' <span class="vis-badge vis-private">● PRIVATE</span>' : '') + '</td>' +
        countCell +
...
function renderPostDetail(view, id) {
  ...
  hdr.innerHTML =
    '<div style="font-family:var(--font-mono); font-size:10px; letter-spacing:0.22em; color:var(--ink-faint); text-transform:uppercase; margin-bottom:4px;">● 자유게시판 / OPEN BOARD ' + visibilityBadge(p, 'post') + '</div>' +
    '<div class="detail-header-row">' +
  ...
  if (editable) {
    ...
    if (delBtn) delBtn.onclick = function() {
      showConfirm('자료 삭제', '「' + x.title + '」 자료를 삭제합니다.\n(업로드된 파일은 저장소에서도 완전히 제거되며, 외부 링크의 경우 원본은 그대로 유지됩니다)', '삭제').then(function(v) {
        if (!v) return;
        // Remove storage file if it was uploaded (not a link)
...
function openCreateRoomModal() {
  ...
  box.querySelector('#room-create').onclick = async function() {
    ...
    saveEntity('chatroom', roomId);

    // Upsert members to DB (handles duplicate-key conflicts gracefully)
    try {
      var memberRows = allMemberIds.map(function(aid) {
        return { room_id: roomId, agent_id: aid, last_read_at: now };
      });
      await sb.from('chat_room_members').upsert(memberRows, {
...
document.addEventListener('focusin', function(e) {
  var t = e.target;
  if (t && t.classList && t.classList.contains('block-editable')) {
    showFormatToolbar(t);
  }
});
...
document.addEventListener('focusout', function(e) {
  ...
  setTimeout(function() {
    ...
    if (ae.classList && ae.classList.contains('block-editable')) return;
